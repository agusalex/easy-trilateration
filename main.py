...
if __name__ == '__main__':
    ...
    parser.add_argument('--file', nargs='?', help='data filename',
                        default='resources/simulation_final/75_75.csv')

    args = parser.parse_args()

    _filename = args.file
    path_groups = _filename.split("/")
    default_simulation = {"actual": [], "min_nodes": 4, "node_location": {
    }}
    try:
    ...
    millis = df['millis'].iloc[0]  # Use .iloc[0] instead of df['millis'][0] for safety

    group_by_node = df.groupby(['node'])
    convert = group_by_node.filter(lambda x: True)  # This line doesn't change anything

    allowed = [1, 3, 6, 8]

    # First filter: Keep only rows where 'node' is in the allowed list
    filtered_by_nodes = df[df['node'].isin(allowed)]

    # Second filter: Ensure each 'millis' group has at least `min_nodes` unique nodes
    group_by_millis_filter = filtered_by_nodes.groupby('millis').filter(
        lambda x: len(pd.unique(x['node'])) >= min_nodes
    )
    ...
    _a = static(history, actual, title=f"Multilateracion {_filename.split('/')[-1]} 4 nodos")
